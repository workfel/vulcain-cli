...
export class CloneCommand 
{
     ...
     /*   this.commandExists('git', (err, exists) => 
        {
            ...
            console.log();
            console.log("*** Get project informations from jellyfish at " + this.options.server + "...");
            ...
            request.end( response => 
                {
                    if( response.ok) 
                    {
                        ...
                        {
                            let env = process.env["JELLYFISH_PROJECT"];
                            if(!env) {
	...
	private commit(dir:string, info) : Q.Promise<boolean>
	{
        ...
        console.log();
		console.log("*** Registering project in jellyfish...");
